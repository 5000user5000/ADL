...
def parse_args():
    ...
    parser.add_argument(
        "--context_file", type=str, default="./data/context.json", help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--max_length",
        type=int,
...
def main():
    ...
    with open(args.context_file, 'r', encoding='utf-8') as context_f:
            raw_context: List = json.load(context_f)

    print("=============cotext=========")
    print(len(raw_context))
    ...
    def preprocess_function(examples):
        ...
        length_parag = len(examples["paragraphs"])
        second_sentences = examples["paragraphs"]
        #second_sentences = [ raw_context[example] for idx in range(length_parag) for example in examples["paragraphs"][idx] ]
        labels =[ examples["paragraphs"][i].index(idx) for i, idx in enumerate(examples["relevant"]) ]
        print("=========================== 3 set over")
